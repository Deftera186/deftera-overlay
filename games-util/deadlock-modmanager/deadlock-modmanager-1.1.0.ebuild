...
src_compile() {
	...
	#    libring_core_0_17_14_.a from ring's perlasm pipeline,
	#    libaws_lc_0_42_0_crypto.a from aws-lc-sys' CMake build and
	...
	#    ring's, unrar_sys' and aws-lc-sys' build.rs to the final link
	#    line, even though the matching `-L` search paths are
	#    propagated. We therefore force the missing static libs in by
	#    hand.
	#
	...
	cargo rustc --release --features tauri/custom-protocol \
		...
		-C link-arg=-lunrar \
		-C link-arg=-laws_lc_0_42_0_crypto \
		-C link-arg=-lc++ \
		-C link-arg=-lgcc \
